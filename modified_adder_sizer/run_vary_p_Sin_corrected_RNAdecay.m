...
base_dir = 'D:\Debu Simulations\Sep 2020\Test';
...
for k = seed_range
	for j = 1:length(p)
		for i = length(Sin):-1:1
			...
			save(strcat(base_dir,'\var_p_sin\','var_p',num2str(p(j)),'_Sin',num2str(Sin(i)),'_rng',num2str(k),'.mat'),'div_durs','div_durs_exp','proto_cell','sim_vars','config','size_bir_exp','size_div_exp','mrna_beg','mrna_end','mrna_produced','prot_beg','prot_end','prot_produced');
		end
	end
end
