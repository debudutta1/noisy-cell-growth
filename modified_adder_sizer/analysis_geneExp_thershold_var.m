		...
		for j = 1:length(t_off_range)
			for l = 1:length(t_on_range)					
				...
				for k = seed_range
					...
					data_compiled{q,i,j,l}.div_durs_exp = [data_compiled{q,i,j,l}.div_durs_exp, data{q,i,j,l,k+1}.div_durs_exp];
					data_compiled{q,i,j,l}.size_bir_exp = [data_compiled{q,i,j,l}.size_bir_exp; data{q,i,j,l,k+1}.size_bir_exp];
					data_compiled{q,i,j,l}.size_div_exp = [data_compiled{q,i,j,l}.size_div_exp; data{q,i,j,l,k+1}.size_div_exp];
				end
			end
		end
	end
...
for q = 1:length(p_range)
	for i = 1:length(threshold_range)
		for j = 1:length(t_off_range)
			for l = 1:length(t_on_range)
				interdiv_stats(1,i,j,l) = mean(data_compiled{i,j,l}.div_durs_exp);
				interdiv_stats(2,i,j,l) = std(data_compiled{i,j,l}.div_durs_exp);
				interdiv_stats(3,i,j,l) = skewness(data_compiled{i,j,l}.div_durs_exp);
				interdiv_stats(4,i,j,l) = kurtosis(data_compiled{i,j,l}.div_durs_exp);
				
				doubDat = data_compiled{i,j,l}.div_durs_exp./log2(mean(data_compiled{i,j,l}.size_div_exp)./mean(data_compiled{i,j,l}.size_bir_exp));
				doubling_stats(1,i,j,l) = mean(doubDat);
				doubling_stats(2,i,j,l) = std(doubDat);
				doubling_stats(3,i,j,l) = skewness(doubDat);
				doubling_stats(4,i,j,l) = kurtosis(doubDat);
				
				doubDat_std = (doubDat-doubling_stats(1,i,j,l))/doubling_stats(2,i,j,l);
				doubling_std_stats(1,i,j,l) = mean(doubDat_std);
				doubling_std_stats(2,i,j,l) = std(doubDat_std);
				doubling_std_stats(3,i,j,l) = skewness(doubDat_std);
				doubling_std_stats(4,i,j,l) = kurtosis(doubDat_std);
			end
		end
	end
end
...
%		for l = 1:length(t_on_range)
			%figure; hold on;
			doubDat = data_compiled{i,j,l}.div_durs_exp./log2(mean(data_compiled{i,j,l}.size_div)./mean(data_compiled{i,j,l}.size_bir));
			histogram((doubDat-doubling_stats(1,i,j,l))/doubling_stats(2,i,j,l),'normalization','pdf','BinWidth',0.25,'Displayname',strcat('threshold=',num2str(threshold_range(i),'%.0e')));
			pd{i} = fitdist(((doubDat-doubling_stats(1,i,j,l))/doubling_stats(2,i,j,l))','GeneralizedExtremeValue');
			xl = xlim; yl = ylim;
...
%		for l = 1:length(t_on_range)
			%figure; hold on;
			doubDat = data_compiled{i,j,l}.div_durs_exp./log2(mean(data_compiled{i,j,l}.size_div)./mean(data_compiled{i,j,l}.size_bir))/60;
			histogram(doubDat,'normalization','pdf','Displayname',strcat('threshold=',num2str(threshold_range(i),'%.0e')));
