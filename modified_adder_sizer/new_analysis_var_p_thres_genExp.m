...
%===============
%% INTER-DIVISION TIMES and not DOUBLING TIMES
...
%		for l = 1:length(t_on_range)
			%figure; hold on;
			doubDat = data_compiled{q,i,j,l}.div_durs_exp;
			histogram((doubDat-interdiv_stats(1,q,i,j,l))/interdiv_stats(2,q,i,j,l),'normalization','pdf','BinWidth',0.25,'Displayname',strcat('threshold=',num2str(threshold_range(i),'%.0e')));
			pd{i} = fitdist(((doubDat-interdiv_stats(1,q,i,j,l))/interdiv_stats(2,q,i,j,l))','GeneralizedExtremeValue');
			xl = xlim; yl = ylim;
...
ylabel('PDF')
title({'Histogram of standardized Generation times when Threshold value is altered',''})
...
%	for j = 1:length(t_off_range)
		...
		for l = length(t_on_range):-1:1
			%figure; hold on;
			doubDat = data_compiled{q,i,j,l}.div_durs_exp;
			doubDat_std = (doubDat-interdiv_stats(1,q,i,j,l))/interdiv_stats(2,q,i,j,l);
...
%===============
% Plot Original value histograms
%===============
...
%		for l = 1:length(t_on_range)
			%figure; hold on;
			doubDat = data_compiled{q,i,j,l}.div_durs_exp/60;
...
xlabel('Doubling time (in mins)')
ylabel('PDF')
title({strjoin({'Histogram of Doubling times varying metab threshold, p =',num2str(p_range(q))},' '),''})
legend('location','northeast')
%xlim([0 200])
...
xlabel('Generation time (in mins)')
ylabel('PDF')
title({'Histogram of Generation times when t_{OFF} is altered',''})
legend('location','northeast')
xlim([0 200])
...
for j = 1:length(t_off_range)
	for l = 1:length(t_on_range)
		plot(threshold_range, reshape(interdiv_stats(1,1,:,j,l),length(threshold_range),[])/60,'-o','linewidth',2,'displayname',strcat('ON',num2str(t_on_range(l)),'OFF',num2str(t_off_range(j))));
		%errorbar(threshold_range, doubling_stats(1,:,j,l)'/60, doubling_stats(2,:,j,l)'/60,'-o','linewidth',2,'MarkerSize',10);
		%errorbar(threshold_range, interdiv_stats(1,:,j,l)'/60, interdiv_stats(2,:,j,l)'/60,'-o','linewidth',2,'MarkerSize',10);
	end
end
xlabel('Increasing metabolite threshold')
ylabel('Mean generation time (in mins)')
title('Varying gene exp and threshold vs generation time')
...
%========================
% Compare Std dist of var p_range
...
xlabel('Standardized Scale')
ylabel('PDF')
title({'Histogram of Generation times when p is varying',''})
legend('location','northeast')
...
figure; hold on;
	...
	lw = length(data_compiled{q,i,j,l}.div_durs_exp);
	doubDat = max(data_compiled{q,i,j,l}.div_durs_exp(randi(lw,1,10000)),data_compiled{q,i,j,l}.div_durs_exp(randi(lw,1,10000)));
	doubDat = max(doubDat,data_compiled{q,i,j,l}.div_durs_exp(randi(lw,1,10000)));
	doubDat = max(doubDat,data_compiled{q,i,j,l}.div_durs_exp(randi(lw,1,10000)));
	doubDat = max(doubDat,data_compiled{q,i,j,l}.div_durs_exp(randi(lw,1,10000)));
	
	doubDat_std = (doubDat-mean(doubDat))/std(doubDat);
	histogram(doubDat_std,'normalization','pdf','Displayname',strcat('t_{OFF}= ',num2str(t_off_range(j))));
	pd{i} = fitdist(doubDat_std','GeneralizedExtremeValue');
	xl = xlim; yl = ylim;
	xval = xl(1):diff(xl)/1000:xl(2); 
...
figure; hold on;
	doubDat = data_compiled{q,i,j,l}.div_durs_exp;
	histogram(doubDat,'normalization','pdf','Displayname',strcat('t_{OFF}= ',num2str(t_off_range(j))));
	pd{i} = fitdist(doubDat','GeneralizedExtremeValue');
	xl = xlim; yl = ylim;
	xval = xl(1):diff(xl)/1000:xl(2); 
	%xval = -2:0.05:12; 
	...
	lw = length(data_compiled{q,i,j,l}.div_durs_exp);
	doubDat = max(data_compiled{q,i,j,l}.div_durs_exp(randi(lw,1,100000)),data_compiled{q,i,j,l}.div_durs_exp(randi(lw,1,100000)));
	histogram(doubDat,'normalization','pdf','Displayname',strcat('t_{OFF}= ',num2str(t_off_range(j))));
	pd{i} = fitdist(doubDat','GeneralizedExtremeValue');
	xl = xlim; yl = ylim;
	xval = xl(1):diff(xl)/1000:xl(2); 
