...
function dx = ODE_gen_MMfn_analysis_noFeed(t,x, ncells, p, n, Sin, par, metab_threshold, cell_dat)
...
% 	metab_threshold = 999*ones(1,p);
	...
	for cell = 1:ncells
		...
		V(first_V(cell)+[1:p]) = x(first_S(cell))./( x(first_S(cell)) + par.KM(:,1) );
		
		% Subsequent Steps in cascade
		if n > 1
			V(first_V(cell)+p+[1:p*(n-1)]) = x(first_S(cell)+[1:p*(n-1)])./( x(first_S(cell)+[1:p*(n-1)]) + reshape(par.KM(:,2:n),[],1) );
		end
	end
% 	V' 
% 	M
