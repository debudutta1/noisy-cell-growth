...
function y = parallel_growth_sim_v2_1(gens, Sin, n,p,T, config, sec_or_min, createRec, threshold, same_thresh)
	...
	child = 1;
	proto_cell{1} = cell_growth_fn_v2_1(n,p,T,Sin, clock, sec_or_min, zero_cell_dat, child, config, createRec, threshold, same_thresh) ;
	...
	for i = 2:proto_n
		...
		proto_cell{i} = cell_growth_fn_v2_1(n,p,T,Sin, clock, sec_or_min, next_dat, child, config, createRec, threshold, same_thresh) ;
	...
	cell_rec{1} = cell_growth_fn_v2_1(n,p,T,Sin, clock, sec_or_min, next_dat, child, config, createRec, threshold, same_thresh) ;
	...
	for i = 1:(gens-1)
		parfor j = 1:(2^i)
			...
			new_cell_rec{j} = cell_growth_fn_v2_1(n,p,T,Sin, clock, sec_or_min, next_dat, child, config, createRec, threshold, same_thresh) ;
