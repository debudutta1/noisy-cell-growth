function bac = cell_growth_fn_ALT_auxFeedSecrete(n,p,T,Sin,clock, sec_or_min, cell_rec, child, config, createRec, threshold, same_thresh, genExp, n_ext, feed_rate, cell_type, secRatio, OVprod_mode)
	...
	while isempty(sol.xe)
		...
		tspan = [clock clock+T];
		sol = ode15s(@(t,x) ODE_gen_MM_auxFeedSecrete_ALT(t,x, 1, p, n, Sin, par, metab_threshold, cell_dat(1), n_ext), tspan, x0, opti2);
		count = count + 1;
		sol2{count} = sol;
