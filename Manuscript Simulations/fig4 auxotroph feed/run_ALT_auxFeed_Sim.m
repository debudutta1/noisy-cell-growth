	...
	for i = 1:length(Sin)
		for j = length(feed_rate):-1:1
			for l = 1:length(secRatio)
				...
				save(strcat(base_dir,'\ALT_aux1_Sin',num2str(Sin(i)),'_secR',num2str(secRatio(l)),'_feed',num2str(feed_rate(j)),'_rng',num2str(k),'.mat'),'div_durs_exp','sim_vars','config','size_bir_exp','size_div_exp');
			end
		end
	end
end
