% Run ALTERNATIVE Auxotrophic Feed and Secretion Simulations
...
if OVprod_mode > 1
	...
	%genExp.t_ON_ov = 5;
	%genExp.t_OFF_ov = 4;
	% par6	Per Cell Protein Ratio 2:1
	%genExp.t_ON_ov = 7;
	%genExp.t_OFF_ov = 29;
	% par7	Population Protein Ratio 2:1
	%genExp.t_ON_ov = 9;
	%genExp.t_OFF_ov = 24;
end
...
% LAB PC
	...
	addpath('C:\Users\Dibyendu_Lab\Google Drive\crossfeed_code\universal code_April2019\crossfeeding_ODE\cleaned final code\MM\Pure_Adder\New folder\modified_adder_sizer\Auxotroph External feed');
	base_dir = 'E:\Debu_Work\Dec 2019';

mkdir(strcat(base_dir,'\var_ALT_auxFeedSec'));
mkdir(strcat(base_dir,'\var_ALT_auxFeedSec\sizer'));
if OVprod_mode == 1
	mkdir(strcat(base_dir,'\var_ALT_auxFeedSec\sizer\1_fluxDouble'));
	base_dir = strcat(base_dir,'\var_ALT_auxFeedSec\sizer\1_fluxDouble');
elseif OVprod_mode == 2
	mkdir(strcat(base_dir,'\var_ALT_auxFeedSec\sizer\2_enzDouble_par2'));
	base_dir = strcat(base_dir,'\var_ALT_auxFeedSec\sizer\2_enzDouble_par2');
elseif OVprod_mode == 3
	mkdir(strcat(base_dir,'\var_ALT_auxFeedSec\sizer\3_FluxEnzDoub'));
	base_dir = strcat(base_dir,'\var_ALT_auxFeedSec\sizer\3_FluxEnzDoub');
end

diary ALT_auxFeedSec.log
	...
	for i = 1:length(Sin)
		for j = length(feed_rate):-1:1
			for l = 1:length(secRatio)
				...
				y = parallel_growth_sim_ALT_auxFeedSecrete(gens, Sin(i), n, p, T, config, sec_or_min, createRec, threshold, same_thresh, genExp, n_ext, feed_rate(j), cell_type1, secRatio(l), OVprod_mode);
				...
				save(strcat(base_dir,'\ALT_aux1_Sin',num2str(Sin(i)),'_secR',num2str(secRatio(l)),'_feed',num2str(feed_rate(j)),'_rng',num2str(k),'.mat'),'div_durs','div_durs_exp','sim_vars','config','size_bir','size_div','size_bir_exp','size_div_exp','secretion_prof','all_metab','mrna_beg','mrna_end','mrna_produced','prot_beg','prot_end','prot_produced','sec_end');
			end
		end
	end
end
