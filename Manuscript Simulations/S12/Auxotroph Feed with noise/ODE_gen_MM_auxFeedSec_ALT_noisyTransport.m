...
function dx = ODE_gen_MM_auxFeedSec_ALT_noisyTransport(t,x, ncells, p, n, Sin, par, metab_threshold, cell_dat, transporter, n_ext)
	
	% MATLAB supposedly passes by reference by default if the variable is never modified in the function!
	ET = zeros(ncells,p,n);
	T = zeros(ncells,1);	% Transporters
	...
	for ncell = 1:ncells
		...
		T(ncell) = interpl_lastval(transporter.prot_dat{1,1}.cum_prot_profile.t, transporter.prot_dat{1,1}.cum_prot_profile.v, t);
	end
	...
	for ncell = 1:ncells
		...
		M(first_S(ncell)+n*p+1, first_V(ncell)) = T*par.k3(1,1)*par.feed_rate; % Since Missing metabolite is placed as the last term always! first_V(ncell) since directly imported without any additional dependent variable
