% Include Noisy Transport

function bac = cell_growth_fn_ALT_auxFeedSec_noisyTransport(n,p,T,Sin,clock, sec_or_min, cell_rec, child, config, createRec, threshold, same_thresh, genExp, n_ext, feed_rate, cell_type, secRatio, OVprod_mode)
	...
	x0 = cell_rec.x0;
	size_bir = x0(each_cell-p0+(1:p0));

	% Initialization of enz_profile variable
	cell_dat{1} = struct('burst_dat',{cell(p,n)},	'mrna_dat',{cell(p,n)},	'prot_dat', {cell(p,n)}, 'mrna_end',cell_rec.mrna_end,	'prot_end',cell_rec.prot_end, 'clock',clock, 'parent',cell_rec.parent, 'birth_time',cell_rec.div_t, 'type',cell_type);
	
	transporter{1} = struct('burst_dat',{cell(1,1)},	'mrna_dat',{cell(1,1)},	'prot_dat', {cell(1,1)}, 'mrna_end',cell_rec.T_mrna_end,	'prot_end',cell_rec.T_prot_end, 'clock',clock, 'parent',cell_rec.parent, 'birth_time',cell_rec.div_t, 'type',cell_type);
	
	% ODE settings
	opti1 = odeset('Events',@(t,x) cell_div_event(t,x,ncells,p0,n,metab_threshold, each_cell),'NormControl','on','RelTol',1e-4);
	opti2 = odeset(opti1,'NonNegative',1:each_cell,'JPattern',JPattern_MM_auxSecrete_ALT(ncells,n,p0,cell_type));	% Auxotrophy
	...
	while isempty(sol.xe)
		...
		cell_dat{1} = enz_prof_fn6_OV_ALT(n,p,T,gene_seqlen,speed_transcription, genExp, prot_seqlen, speed_translation, clock, extend, cell_dat{1}, singOper, OVprod_mode);
		
		transporter{1} = enz_prof_fn6_OV_ALT(1,1,T,gene_seqlen,speed_transcription, genExp, prot_seqlen, speed_translation, clock, extend, transporter{1}, singOper, OVprod_mode);
		
		% Generate and Simulate the ODE function for metabolite production, and cross-feeding
		tspan = [clock clock+T];
		sol = ode15s(@(t,x) ODE_gen_MM_auxFeedSec_ALT_noisyTransport(t,x, 1, p, n, Sin, par, metab_threshold, cell_dat(1), transporter{1}, n_ext), tspan, x0, opti2);
		count = count + 1;
		sol2{count} = sol;
	...
	% Transporter mRNA and protein
	i = 1; j = 1;
	T_mrna_end = interpl_lastval(transporter{1}.mrna_dat{i,j}.tot_rna_profile.t, transporter{1}.mrna_dat{i,j}.tot_rna_profile.v, div_t);
	T_prot_end = interpl_lastval(transporter{1}.prot_dat{i,j}.tot_prot_profile.t, transporter{1}.prot_dat{i,j}.tot_prot_profile.v, div_t);
	
	% Create cell_rec for later analysis of data (createRec == 1 => Detailed record of profile. 0 => basic record)
	cell_rec = compile_cell_rec_v2(n,p,cell_dat{1}.birth_time, div_t, cell_dat{1}, sol2, mrna_end, prot_end, ES_ratio, cell_rec.parent, cell_rec.mrna_end,cell_rec.prot_end, createRec);
	...
	bac.progeny.x0 = zeros(each_cell+1,2);
...
%	part_ratio = binornd(1000,0.5)/1000;
	...
	% Transporter mRNA
	bac.progeny.T_mrna(1) = round(part_ratio*T_mrna_end);
	bac.progeny.T_mrna(2) = T_mrna_end - bac.progeny.T_mrna(1);
	
	% prot distribution
	bac.progeny.prot(:,:,1) = round(part_ratio*cell_rec.prot_end);
	bac.progeny.prot(:,:,2) = cell_rec.prot_end - bac.progeny.prot(:,:,1);
	% Transporter prot
	bac.progeny.T_prot(1) = round(part_ratio*T_prot_end);
	bac.progeny.T_prot(2) = T_prot_end - bac.progeny.T_prot(1);
	
	% metabolite distribution
	if adder == 1
		% Pure Adder, reset final metabolites to ZERO
		x0(each_cell-p0+(1:p0),:) = 0;	
	elseif adder == 0
		% Metabolic Sizer, metab_threshold amount gets used up, rest is passed on to the daughter cells
		x0(each_cell-p0+(1:p0)) = x0(each_cell-p0-1+(1:p0)) - metab_threshold(:); 
	...
	% Set Counter of secretion to ZERO for next cell's count
	x0(end) = 0;
	
	%bac.secretion_prof = [sol.x; sol.y(end,:)];
	bac.sec_end = sol.y(end,end);
	bac.all_metab = sol.ye;
	
	bac.progeny.x0(:,1) = part_ratio*x0;
	bac.progeny.x0(:,2) = x0 - bac.progeny.x0(:,1);
	
	bac.seed_store = seed_store;
	bac.size_div = x0(each_cell-p0+(1:p0));
