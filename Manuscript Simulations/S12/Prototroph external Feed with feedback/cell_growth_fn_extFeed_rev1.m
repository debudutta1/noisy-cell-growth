function bac = cell_growth_fn_extFeed_rev1(n,p,T,Sin,clock, sec_or_min, cell_rec, child, config, createRec, threshold, same_thresh, t_on, t_off, n_ext, feed_rate, k3_var) 	
	...
	while isempty(sol.xe)
		...
		tspan = [clock clock+T];
		sol = ode15s(@(t,x) ODE_gen_MMfn_analysis_extFeed(t,x, 1, p, n, Sin, par, metab_threshold, cell_dat(1), n_ext), tspan, x0, opti2);
		count = count + 1;
		sol2{count} = sol;
