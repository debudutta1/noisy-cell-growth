% Run ext Feed Simulations with varying k3 - Reviewer 1 suggestion
...
k3_var_range = [1 2.5 5 10 25 50 90];
...
mkdir(strcat(base_dir,'\var_extFeed_Rev1'));
base_dir = 'D:\Debu Simulations\Sep 2020\var_extFeed_Rev1\';

for k3_var = k3_var_range
		...
		for n_ext = n_ext_range
			...
			for i = 1:length(Sin)
				for k = seed_range
					...
					y = parallel_growth_sim_extFeed_rev1(gens, Sin(i), n, p, T, config, sec_or_min, createRec, threshold, same_thresh, t_ON, t_OFF, n_ext, feed_rate(j), k3_var);
					...
					save(strcat(base_dir,'var_Sin',num2str(Sin(i)),'_nExt',num2str(n_ext),'_feed',num2str(feed_rate(j)),'_rng',num2str(k),'_k3feedbk_',num2str(k3_var),'.mat'),'div_durs_exp','sim_vars','config','size_bir_exp','size_div_exp');
