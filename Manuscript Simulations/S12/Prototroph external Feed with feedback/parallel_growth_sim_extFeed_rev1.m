...
function y = parallel_growth_sim_extFeed_rev1(gens, Sin, n,p,T, config, sec_or_min, createRec, threshold, same_thresh, t_on, t_off, n_ext, feed_rate, k3_var)
	...
	child = 1;
	proto_cell{1} = cell_growth_fn_extFeed_rev1(n,p,T,Sin, clock, sec_or_min, zero_cell_dat, child, config, createRec, threshold, same_thresh, t_on, t_off, n_ext, feed_rate, k3_var) ;
	...
	for i = 2:proto_n
		...
		proto_cell{i} = cell_growth_fn_extFeed_rev1(n,p,T,Sin, clock, sec_or_min, next_dat, child, config, createRec, threshold, same_thresh, t_on, t_off, n_ext, feed_rate, k3_var) ;
	...
	cell_rec{1} = cell_growth_fn_extFeed_rev1(n,p,T,Sin, clock, sec_or_min, next_dat, child, config, createRec, threshold, same_thresh, t_on, t_off, n_ext, feed_rate, k3_var) ;
	...
	for i = 1:(gens-1)
		parfor j = 1:(2^i)
			...
			new_cell_rec{j} = cell_growth_fn_extFeed_rev1(n,p,T,Sin, clock, sec_or_min, next_dat, child, config, createRec, threshold, same_thresh, t_on, t_off, n_ext, feed_rate, k3_var) ;
