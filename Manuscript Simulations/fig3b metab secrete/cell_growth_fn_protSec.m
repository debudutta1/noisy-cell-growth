...
function bac = cell_growth_fn_protSec(n,p,T,Sin,clock, sec_or_min, cell_rec, child, config, createRec, threshold, same_thresh, secRatio) 	
	...
	while isempty(sol.xe)
		...
		tspan = [clock clock+T];
		sol = ode15s(@(t,x) ODE_gen_MMfn_analysis_protSec(t,x, 1, p, n, Sin, par, metab_threshold, cell_dat(1), secRatio), tspan, x0, opti2);
		count = count + 1;
		sol2{count} = sol;
