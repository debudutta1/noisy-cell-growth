...
function y = parallel_growth_sim_noisy_cat(gens, Sin, n,p,T, config, sec_or_min, createRec, threshold, same_thresh)
	...
	zero_cell_dat = struct('mrna_end',zeros(p,n),'prot_end',zeros(p,n),'parent',0,'birth_time',clock, 'x0', zeros(1, each_cell), 'index', 0, 'div_t', clock,'cat_mrna_end',0,'cat_prot_end',0);
	child = 1;
	proto_cell{1} = cell_growth_fn_noisy_cat(n,p,T,Sin, clock, sec_or_min, zero_cell_dat, child, config, createRec, threshold, same_thresh) ;
	...
	for i = 2:proto_n
		...
		next_dat = struct('mrna_end',proto_cell{i-1}.progeny.mrna(:,:,1),	'prot_end',proto_cell{i-1}.progeny.prot(:,:,1), 'parent',i-1, 'x0', proto_cell{i-1}.progeny.x0(:,1), 'index', i-1, 'div_t', proto_cell{i-1}.div_t, 'cat_mrna_end',proto_cell{i-1}.progeny.cat_mrna(1),	'cat_prot_end',proto_cell{i-1}.progeny.cat_prot(1));
		
		proto_cell{i} = cell_growth_fn_noisy_cat(n,p,T,Sin, clock, sec_or_min, next_dat, child, config, createRec, threshold, same_thresh) ;
	...
	% Create input for cell of next generation
	next_dat = struct('mrna_end',proto_cell{proto_n}.progeny.mrna(:,:,1),	'prot_end',proto_cell{proto_n}.progeny.prot(:,:,1), 'parent',i-1, 'x0', proto_cell{proto_n}.progeny.x0(:,1), 'index', i-1, 'div_t', proto_cell{proto_n}.div_t, 'cat_mrna_end',proto_cell{i-1}.progeny.cat_mrna(1),	'cat_prot_end',proto_cell{i-1}.progeny.cat_prot(1));
		
	cell_rec{1} = cell_growth_fn_noisy_cat(n,p,T,Sin, clock, sec_or_min, next_dat, child, config, createRec, threshold, same_thresh) ;
	...
	for i = 1:(gens-1)
		parfor j = 1:(2^i)
			...
			% Create input for cell of next generation
			next_dat = struct('mrna_end',cell_rec{parent}.progeny.mrna(:,:,progeny),	'prot_end',cell_rec{parent}.progeny.prot(:,:,progeny), 'parent',parent, 'x0', cell_rec{parent}.progeny.x0(:,progeny), 'index', index, 'div_t', cell_rec{parent}.div_t, 'cat_mrna_end',cell_rec{parent}.progeny.cat_mrna(1),	'cat_prot_end', cell_rec{parent}.progeny.cat_prot(1));
			
			clock = cell_rec{parent}.div_t;
			new_cell_rec{j} = cell_growth_fn_noisy_cat(n,p,T,Sin, clock, sec_or_min, next_dat, child, config, createRec, threshold, same_thresh) ;
