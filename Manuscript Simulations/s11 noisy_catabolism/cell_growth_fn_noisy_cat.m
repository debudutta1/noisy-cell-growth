% Version V2_1 modified from V2, to include new compile_cell_rec_v2 function which will accept createRec == 2 and output mrna/prot produced data

function bac = cell_growth_fn_noisy_cat(n,p,T,Sin,clock, sec_or_min, cell_rec, child, config, createRec, threshold, same_thresh) 	
	...
	each_cell = each_cell + 1;	% for noisy catabolism extra variable
	...
	catabol{1} = struct('burst_dat',{cell(1,1)},	'mrna_dat',{cell(1,1)},	'prot_dat', {cell(1,1)}, 'mrna_end',cell_rec.cat_mrna_end,	'prot_end',cell_rec.cat_prot_end, 'clock',clock, 'parent',cell_rec.parent, 'birth_time',cell_rec.div_t);
	
	% ODE settings
	opti1 = odeset('Events',@(t,x) cell_div_event(t,x,ncells,p,n,metab_threshold, each_cell),'NormControl','on','RelTol',1e-3);
%	opti2 = odeset(opti1,'NonNegative',1:each_cell,'OutputFcn',@odeplot);	% No CrossFeeding
	opti2 = odeset(opti1,'NonNegative',1:each_cell,'JPattern',JPattern_noisy_cat(ncells,n,p,0));	% No CrossFeeding
	...
	while isempty(sol.xe)
		...
		catabol{1} = enz_prof_fn3(1,1,T,gene_seqlen,speed_transcription,t_on,t_off, prot_seqlen, speed_translation, clock, extend, catabol{1}, singOper);
		
		% Generate and Simulate the ODE function for metabolite production, and cross-feeding
		tspan = [clock clock+T];
		sol = ode15s(@(t,x) ODE_gen_noisy_cat(t,x, 1, p, n, Sin, par, metab_threshold, cell_dat(1), catabol{1}), tspan, x0, opti2);
		count = count + 1;
		sol2{count} = sol;
...
%		end
	...
	% Catabol mRNA and protein
	i = 1; j = 1;
	cat_mrna_end = interpl_lastval(catabol{1}.mrna_dat{i,j}.tot_rna_profile.t, catabol{1}.mrna_dat{i,j}.tot_rna_profile.v, div_t);
	cat_prot_end = interpl_lastval(catabol{1}.prot_dat{i,j}.tot_prot_profile.t, catabol{1}.prot_dat{i,j}.tot_prot_profile.v, div_t);
...
%	part_ratio = binornd(1000,0.5)/1000;	% Model Default almost Equivalent to Equal ratio, since CV ~ 0.03 only
	
	% Exact 1:1 partition
	part_ratio = 0.5;
	
	% Truncated normal with CV = 0.1
%	pd = makedist('Normal',0.5,0.1*0.5);	% Yields a partition ratio with CV ~ 0.1
%	pd = truncate(pd, 0, 1);
%	part_ratio = random(pd);
	
	% mrna distribution
	bac.progeny.mrna(:,:,1) = round(part_ratio*cell_rec.mrna_end);
	bac.progeny.mrna(:,:,2) = cell_rec.mrna_end - bac.progeny.mrna(:,:,1);
	% Catabol mRNA
	bac.progeny.cat_mrna(1) = round(part_ratio*cat_mrna_end);
	bac.progeny.cat_mrna(2) = cat_mrna_end - bac.progeny.cat_mrna(1);
	
	% prot distribution
	bac.progeny.prot(:,:,1) = round(part_ratio*cell_rec.prot_end);
	bac.progeny.prot(:,:,2) = cell_rec.prot_end - bac.progeny.prot(:,:,1);
	% Catbol prot
	bac.progeny.cat_prot(1) = round(part_ratio*cat_prot_end);
	bac.progeny.cat_prot(2) = cat_prot_end - bac.progeny.cat_prot(1);
	...
	bac.burst_dat = cell_dat{1}.burst_dat;
end