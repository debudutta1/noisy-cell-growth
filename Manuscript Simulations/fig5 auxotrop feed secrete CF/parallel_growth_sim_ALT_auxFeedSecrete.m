...
function y = parallel_growth_sim_ALT_auxFeedSecrete(gens, Sin, n,p,T, config, sec_or_min, createRec, threshold, same_thresh, genExp, n_ext, feed_rate, cell_type, secRatio, OVprod_mode)
	...
	zero_cell_dat = struct('mrna_end',zeros(p1,n),'prot_end',zeros(p1,n),'parent',0,'birth_time',clock, 'x0', zeros(1, each_cell), 'index', 0, 'div_t', clock, 'type', cell_type);
	child = 1;
	proto_cell{1} = cell_growth_fn_ALT_auxFeedSecrete(n,p,T,Sin, clock, sec_or_min, zero_cell_dat, child, config, createRec, threshold, same_thresh, genExp, n_ext, feed_rate, cell_type, secRatio, OVprod_mode) ;
	...
	for i = 2:proto_n
		...
		proto_cell{i} = cell_growth_fn_ALT_auxFeedSecrete(n,p,T,Sin, clock, sec_or_min, next_dat, child, config, createRec, threshold, same_thresh, genExp, n_ext, feed_rate, cell_type, secRatio, OVprod_mode) ;
	...
	cell_rec{1} = cell_growth_fn_ALT_auxFeedSecrete(n,p,T,Sin, clock, sec_or_min, next_dat, child, config, createRec, threshold, same_thresh, genExp, n_ext, feed_rate, cell_type, secRatio, OVprod_mode) ;
	...
	for i = 1:(gens-1)
		parfor j = 1:(2^i)
			...
			new_cell_rec{j} = cell_growth_fn_ALT_auxFeedSecrete(n,p,T,Sin, clock, sec_or_min, next_dat, child, config, createRec, threshold, same_thresh, genExp, n_ext, feed_rate, cell_type, secRatio, OVprod_mode) ;
